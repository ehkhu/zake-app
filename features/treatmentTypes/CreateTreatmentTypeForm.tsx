...
// import { createEditTreatmentType } from "@/services/apiTreatmentTypes";
import { useCreateTreatmentType } from "./useCreateTreatmentType";
...
export function CreateTreatmentTypeForm({ treatmentTypeToEdit = {}}) {
  const { toast } = useToast();
  const [errors, setErrors] = useState<any>({});

  //for edit 
  let idToUpdate = 0;
  let editValues:any = treatmentTypeToEdit;
  ...
  const {createTreatmentType,isCreating} = useCreateTreatmentType();
  const {editTreatmentType,isUpdating} = useEditTreatmentType();
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if(isEditSession){
      editTreatmentType({treatmentType:values,id:idToUpdate},{onSuccess:()=>{
        setErrors({});
      },
      onError: (err) => onError(err)
      });
    }else{
      createTreatmentType(values,{onSuccess:()=>{
        form.reset();
        setErrors({});
      },
      onError: (err) => onError(err)
