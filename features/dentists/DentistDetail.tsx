...
export default function DentistDetail({ anDentistId }: any) {
  const dentistId = anDentistId;
  const { isLoading, dentist, error } = useDentist(dentistId);
      ...
      <div className="flex space-x-2">
        <Card className="w-1/2">
          ...
          <CardContent className=" space-y-4">
            {/* content */}
            <h3 className="text-sm">{dentist.data.name}</h3>
            <Separator className="my-3" />
            <div className="flex items-center space-x-4">
              <Phone className="w-4 h-4 text-muted-foreground" />{" "}
              <span>{dentist.data.contact_number}</span>
            </div>
            <div className="flex items-center space-x-4">
              <Mail className="w-4 h-4 text-muted-foreground" />{" "}
              <span>{dentist.data.email}</span>
            </div>
          </CardContent>
        ...
        <Card className="w-1/2">
          <CardHeader>
            ...
            <CardDescription>{dentist.data.name} activities</CardDescription>
          </CardHeader>
          <CardContent className="p-6 pt-6 flex justify-between text-center content-center">
            <div className="-mx-2 grid justify-items-center p-2 w-1/3">
              ...
              <div className="space-y-2">
                <p className="text-xl font-medium leading-none">
                  {dentist.data.appointments_count}
                </p>
            ...
            <div className="-mx-2 grid justify-items-center space-x-4 p-2 w-1/3">
              ...
              <div className="space-y-2">
                <p className="text-xl font-medium leading-none">
                  {dentist.data.total_treatment_records}
                </p>
            ...
            <div className="-mx-2 grid justify-items-center space-x-4 p-2 w-1/3">
              ...
              <div className="space-y-2">
                <p className="text-xl font-medium leading-none">
                  {dentist.data.total_charge_amount}
                </p>
      ...
      <Card className="w-full">
        ...
        <CardContent>
          <ol className="relative border-s border-gray-200 dark:border-gray-700">
            {dentist.data.appointments.map((appointment: any) => (
